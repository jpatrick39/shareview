...
function patientCard(r) {
  ...
  const spo2 = Number(r.spo2Percent || 0);
  const temp = Number(r.skinTemperatureC || 0);
  const ppg = ppgValues(r);
  return `<article class="patient-card" data-id="${escapeHtml(r.id)}">
      <div class="patient-main">
        <strong>${escapeHtml(r.patientName)}</strong>
        <span>${escapeHtml(r.patientId)} • ${escapeHtml(r.room)}</span><br />
        <span>${timeAgo(r.capturedAtEpochMs)} • ${escapeHtml(r.deviceId || r.watchModel || 'Galaxy Watch')}</span>
      </div>
      <div class="vitals">
        <div class="vital"><span>Est. BP</span><strong>${r.systolic}/${r.diastolic}</strong></div>
        <div class="vital"><span>HR</span><strong>${r.heartRateBpm}</strong></div>
        <div class="vital"><span>SpO₂</span><strong>${spo2 ? `${spo2}%` : '--'}</strong></div>
        <div class="vital"><span>Temp</span><strong>${temp ? `${temp.toFixed(1)}°C` : '--'}</strong></div>
      </div>
      <div class="wave-stack">
        <label>ECG</label>
        <canvas class="ecg" data-ecg='${JSON.stringify(ecgValues(r))}'></canvas>
        <label>PPG</label>
        <canvas class="ppg" data-ppg='${JSON.stringify(ppg)}'></canvas>
      </div>
      <div>
        <span class="badge ${alertLevel}">${badgeText}</span>
        <div class="subtle" style="margin-top:8px">${escapeHtml(r.confidenceLabel || 'Estimate only')}</div>
        <div class="subtle">ECG: ${escapeHtml(r.ecgStatus || 'Captured')}</div>
        <div class="subtle">PPG Samples: ${ppg.length}</div>
      </div>
    </article>`;
}
...
function drawAllEcg() {
  document.querySelectorAll('canvas.ecg').forEach(canvas => drawEcg(canvas, JSON.parse(canvas.dataset.ecg || '[]')));
  document.querySelectorAll('canvas.ppg').forEach(canvas => drawPpg(canvas, JSON.parse(canvas.dataset.ppg || '[]')));
}
...
function drawPpg(canvas, values) {
  const dpr = window.devicePixelRatio || 1;
  const rect = canvas.getBoundingClientRect();
  ...
  ctx.strokeStyle = 'rgba(114, 224, 168, 0.16)';
  ctx.lineWidth = 1;
  ...
  ctx.strokeStyle = '#72e0a8';
  ctx.lineWidth = 2;
  ctx.beginPath();
  nums.forEach((v, i) => {
    const x = (i / Math.max(nums.length - 1, 1)) * rect.width;
...
function ppgValues(r) { if (Array.isArray(r.ppgPreview)) return r.ppgPreview.map(Number).filter(Number.isFinite); if (Array.isArray(r.ppg?.preview)) return r.ppg.preview.map(Number).filter(Number.isFinite); const csv = r.ppgPreviewCsv || r.ppg?.stripPreviewCsv || r.ppgCsv; if (typeof csv === 'string' && csv.trim()) return csv.split(',').map(Number).filter(Number.isFinite).slice(0, 480); return []; }
